...
from src.config.settings import XttsSettings
...
class XttsProvider(BaseTTS):
    ...
    def _extract_word_timestamps(self, words: List[str], attention_weights: np.ndarray, audio_duration: float, n_mel_frames: int) -> List[WordTimestamp]:
        """
    ...
    def _post_process_timestamps(self, timestamps: List[WordTimestamp], audio_duration: float) -> List[WordTimestamp]:
        """
    ...
    def synthesize_speech(self, text: str, output_path: Optional[str] = None) -> Tuple[bytes, List[Tuple[str, float]]]:
        try:
